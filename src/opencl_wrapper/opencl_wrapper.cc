 ...
 * \brief The implementation was taken from Apache TVM project
 */
 ...
 private:
  ...
  void openLibOpenCL() {
    ...
    if (m_libHandler == nullptr) {
        throw std::runtime_error("Error! Cannot open libOpenCL!");
    }
  }
...
// Extended from TVM version:
using f_clCreateContextFromType = cl_context (*)(const cl_context_properties*, cl_device_type,
                                                 f_pfn_notify, void*, cl_int*);
using f_clGetContextInfo = cl_int (*)(cl_context, cl_context_info, size_t, void*, size_t*);
using f_clGetExtensionFunctionAddress = void* (*)(const char*);
using f_clGetKernelWorkGroupInfo = cl_int (*)(cl_kernel, cl_device_id, cl_kernel_work_group_info,
                                              size_t, void*, size_t*);
...
cl_context clCreateContextFromType(const cl_context_properties* properties, cl_device_type device_type,
                                   ...
                                   void* user_data, cl_int* errcode_ret) {
  auto& lib = LibOpenCLWrapper::getInstance();
  auto func = (f_clCreateContextFromType)lib.getOpenCLFunction("clCreateContextFromType");
  if (func) {
...
cl_int clGetContextInfo(cl_context context, cl_context_info param_name, size_t param_value_size,
                        void *param_value, size_t *param_value_size_ret) {
  auto& lib = LibOpenCLWrapper::getInstance();
  auto func = (f_clGetContextInfo)lib.getOpenCLFunction("clGetContextInfo");
  if (func) {
...
void* clGetExtensionFunctionAddress(const char* func_name) {
  auto& lib = LibOpenCLWrapper::getInstance();
  auto func = (f_clGetExtensionFunctionAddress)lib.getOpenCLFunction("clGetExtensionFunctionAddress");
  if (func) {
...
cl_int clGetKernelWorkGroupInfo(cl_kernel kernel, cl_device_id device, cl_kernel_work_group_info param_name,
                                size_t param_value_size, void* param_value, size_t* param_value_size_ret) {
  auto& lib = LibOpenCLWrapper::getInstance();
  auto func = (f_clGetKernelWorkGroupInfo)lib.getOpenCLFunction("clGetKernelWorkGroupInfo");
  if (func) {
